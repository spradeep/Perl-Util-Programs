...
my ($train_info_table) = grep { $_->look_down( sub {
		$_[0]->tag eq 'td' && $_[0]->as_text =~ /journey details/sig
	}) } 
	$html_tree->look_down( sub {
...
if($train_info_table && ref $train_info_table eq 'HTML::Element')
{
	...
	@train_info{qw/name no dt from to rupto bpoint class/} = map {my $t = $_->as_text; $t =~ s/(^\s+|\s+$)//g; $t;} $train_info_table->look_down( sub	{
...
my ($data_table) = $html_tree->look_down( sub {
...
foreach my $tr ($data_table->look_down( sub {
	$_[0]->tag eq 'tr'
})) 
{
	my @tds = $tr->look_down( sub {
		$_[0]->tag eq 'td' && $_[0]->attr('class') eq 'table_border_both'
	});
...
delete @cols[@to_delete]; warn @cols;
